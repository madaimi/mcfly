...
export class NoteController {
    ...
    @Patch('/markFavorite/:noteId')
    async markAsFavorite(@Res() res, @Param("noteId") _id: string){
        const note = await this.noteService.markAsFavorite(_id);
        if(!note)
            return res
                .status(HttpStatus.NOT_FOUND)
                .json({ status: 404, error: "Not found!"});
    ...
    @Get('/allFavorites')
    async getAllFavorites(@Res() res) {
        const notes = await this.noteService.getAllFavorites();
        return res.status(HttpStatus.OK).json({
            status: 200,
            data: notes
        })
